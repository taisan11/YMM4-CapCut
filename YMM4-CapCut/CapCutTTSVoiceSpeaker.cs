...
namespace CapCutTTSVoicePlugin {
    public class CapCutTTSVoiceSpeaker : IVoiceSpeaker {
        ...
        /// 音声合成エンジンの名前（表示用）
        /// </summary>
        public string EngineName => "CapCutTTS";
        ...
        public CapCutTTSVoiceSpeaker(string speaker) => ID = speaker;
        ...
        {
            return new CapCutTTSVoiceParameter();
        }
        ...
        {
            if (currentParameter is not CapCutTTSVoiceParameter)
        ...
        public async Task<IVoicePronounce?> CreateVoiceAsync(string text, IVoicePronounce? pronounce, IVoiceParameter? parameter, string filePath)
        {
            ...
            try
            {
                var param = parameter as CapCutTTSVoiceParameter ?? (CapCutTTSVoiceParameter)CreateVoiceParameter();
                // CapCutAPIClient.TalkAsyncを使用して音声データを取得
                byte[] audioData = await CapCutAPIClient.TalkAsync(text, ID, param.Pitch, param.Speed, param.Volume);
                // 取得した音声データをファイルに保存
